 ...
 age:string=AGE.ANY;
  transform(centers: any[], hospitalName: string,dose:string,vaccineType:any,age:string,activeDate:string): any {
    ...
    checkAge(center:any,age:string,activeDate:string){
     return (age!=AGE.ANY?(this.checkSessions(center[activeDate],'minAgeLimit','===',parseInt(age)) ):true)
    }
    checkDoseType(center:any,doseType:string,activeDate:string){
      return (doseType!=DOSE.ANY?(this.checkSessions(center[activeDate],('availableCapacityDose'+doseType),">",0)):true);
    }

    vaccineType(center:any,vaccineType:VACCINES,activeDate:string){
      return (vaccineType!=VACCINES.ANY?(this.checkSessions(center[activeDate],'vaccine','===',vaccineType)):true);
    }
    ...
    checkSessions(sessions:any,check:string,operator:string,operand:string|number){
      for(let i=0;i<sessions.length;i++){
        switch(operator){
          case ">":
            if(sessions[i] && sessions[i][check] && sessions[i][check]>operand &&  (this.age!==AGE.ANY?(sessions[i].minAgeLimit>=parseInt(this.age)):true)){
              return true;
            }
            break;
          case "===":
